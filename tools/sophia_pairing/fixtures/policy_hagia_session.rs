...
//! Ordinary cases have no output bootstrap. The presentation case supplies
//! explicit test target facts; it never constructs a native device.
use super::*;
...
// Matches the mirrored test target's two heads. This is supplied topology and
// capability metadata; no native inventory, modeset or output peer is exercised.
fn presentation_fixture_bootstrap(
    ...
    LiveOutputAuthorityBootstrap {
        snapshot: OutputAuthoritySnapshot {
            ...
            heads: [1, 2]
                .map(|id| OutputHeadDescriptor {
                    head: DisplayHeadId::from_raw(id),
                    generation: 1,
                    label: format!("Simulated mirror {id}"),
                    connected: true,
                    enabled: true,
                    current_mode: Some(DisplayModeId::from_raw(id)),
...
fn with_normal_hagia_transport_desktop<T>(
    ...
    with_normal_hagia_transport_fixture(case, transport, desktop, false, exercise)
}

fn with_normal_hagia_transport_fixture<T>(
    case: &str,
    transport: WmTransportSelection,
    desktop: Option<&str>,
    ...
    // This selects the existing production capability ceiling. Only the
    // explicitly simulated backend fixture may supply the corresponding facts.
    source.config.native_scanout = presentation_fixture;
    ...
    let bootstrap = presentation_fixture.then(|| presentation_fixture_bootstrap(output));
    let mut wm =
        LiveWmSession::from_started_public_config(&source.config, &[output], started, bootstrap)
            .unwrap();
    let protection = wm
        .supervisor
        .protection_evidence()
    ...
    if presentation_fixture {
        expected_keys.insert(sophia_runtime::SOPHIA_OUTPUT_SOCKET_ENV.to_owned());
        assert!(
            protection
                .roles
    ...
    let presentation_caps = sophia_protocol::SOPHIA_WM_CAPABILITY_SURFACE_INSTANCES
        | sophia_protocol::SOPHIA_WM_CAPABILITY_PRESENTATION_ACTIONS;
    let forbidden = if presentation_fixture {
        0
    } else {
        presentation_caps
    };
    ...
    assert_eq!(public.output_service.is_some(), presentation_fixture);
    assert_eq!(
        selected & presentation_caps,
        if presentation_fixture {
            presentation_caps
        } else {
            0
    ...
    if presentation_fixture {
        assert!(
            configuration
                .actions
                .iter()
                .any(|action| action.name == "toggle-overview")
        );
    } else {
        assert!(
            configuration
                .actions
                .iter()
                .all(|action| action.name != "toggle-overview"
                    && !action.name.starts_with("overview-")),
            "no overview catalog without native retirement capabilities"
        );
    }
