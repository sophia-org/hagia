...
proc validatePayload(kind: MessageKind, payload: openArray[byte]) =
  ...
  of MessageKind.outputActionRequest:
    ...
    payload.requireReserved(66, 2)
    let outputCount = int(payload.u16At(64))
    if outputCount < 1 or outputCount > maxOutputs or payload.len != 68 + outputCount * 8:
      fail(PolicyProtocolErrorKind.fieldTooLarge, "output action coverage is invalid")
