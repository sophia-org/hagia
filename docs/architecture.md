...
Directional navigation stops at outer edges, permits adjacent-monitor handoff
when `arrow-crosses-outputs` is enabled (the default),
...
with the opposite arrow when crossing is enabled. Checkpoint version 19 persists
this preference; older checkpoints migrate with crossing enabled. Explicit
output switching is independent of the preference. Handoff preserves each monitor's selected workspace
