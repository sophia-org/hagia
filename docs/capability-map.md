...
| Scrolling and additional retained layouts | Hagia policy; visible tabs and feedback in Narthex | Native scroller, tile, grid, monocle, and vertical-scroller cycle implemented; the freeze-era structural exclusion (ledger rows 7 and 9) resolved through opaque projection extension records and the persistent tree family (frame-tree, Notion, i3, dwindle); Janet remains a later tranche | Deterministic geometry, constraint, transition, and shell correspondence tests |
