...
| Scrolling and additional retained layouts | Hagia policy; visible tabs and feedback in Narthex | Native scroller, tile, grid, monocle, and vertical-scroller cycle implemented; the structural exclusion recorded at revision 3 (ledger rows 7 and 9) resolved through opaque projection extension records and the persistent tree family (frame-tree, Notion, i3, dwindle); Janet remains a later tranche | Deterministic geometry, constraint, transition, and shell correspondence tests |
...
| Overview arrangement, navigation, and workspace/window selection | Hagia spatial policy; Engine provides generic presentation and physical input mechanisms | h002 prototype; ownership corrected to WM policy on 2026-09-25; generic contract definition is Sophia t242, paired acceptance t241 | Proposed capability-gated WM presentation/input contract; no pixels, renderer handles, or raw input reach Hagia |
| Switcher, panels, tabs, toast, and visible chrome | Separately admitted shell clients, with Engine rendering | Switcher, work-area reservation, and the Tier-0 per-head status strip complete (archives 0005/0006); other shell work retains its existing scope | `sophia_shell_v1`; shell content does not gain arbitrary application-surface access |
| Launch, logout, lock, output power, and configuration | Sophia session or dedicated authority | Terminal, browser, close, and logout slots plus output authority and configuration activation complete (ledger rows, archive 0001); lock and output power remain later work | Advertised opaque operations or role-specific interfaces |
...
| KDL configuration and Janet policy/layouts | Hagia, within bounded policy authority | Retained declarative KDL path complete; Janet excluded from the revision-3 profile | Keep candidate validation and atomic activation stable; model bounded evaluation and deterministic fallback before any Janet addition |
