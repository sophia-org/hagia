...
| Behavior family | State | Port requirement |
| --- | --- | --- |
| Physical key, pointer, axis, gesture, switch, and shortcut matching | Partial | The public shortcut owner now retains its canonical typed candidate in a separate authority-local `Prepared` slot and resolves Hagia's semantic action catalog from that payload into Engine-owned matching without sending raw input to Hagia. Add global activation plus the retained axis, gesture, and switch candidates. |
| Input-device and XKB configuration | Partial | The canonical typed input payload now resides in a cohesive owner record backed by its authority-local `Prepared` slot; existing RMLVO, repeat, lock-state, natural-scrolling, acceleration, handedness, middle-emulation, and wheel-scaling preparation reads that payload. Add global activation, device-scoped capabilities, and cross-authority live prepare/activate/rollback. |
| Output mode, scale, position, transform, VRR, enablement, power, and reservations | Partial | The canonical typed output payload now resides in a cohesive owner record backed by its authority-local `Prepared` slot and reconciles purely against immutable capabilities. The atomic owner exposes read-only libdrm facts, a pure adapter joins them to Engine outputs without overclaiming support, and startup admits the result before client launch without activation. Add atomic multi-output testing/apply, rollback, reservations, and a separate power authority. |
| Launch, startup environment, configured processes, and shell supervision | Partial | Typed terminal/browser/startup/logout selectors resolve only against Sophia's registered applications. A bounded immutable overlay centralizes CLI-superior application additions, arguments, startup order, and action mappings in one pure cloned-registry preparation seam. The canonical session payload now resides in a real authority-local slot that remains `Prepared`; port global activation plus the remaining retained launch environment and supervision behavior. |
...
| Configuration discovery, validation, activation, reload, and rollback | Open | Discovery, partitioning, staging, policy preparation, and typed shortcut preparation exist. The generation/digest barrier retains a monotonic attempt counter and executable TLA+ proofs; Sophia carries the matching coordinator, exact-key participant reducer, and separate prepare-only startup driver. Public Hagia startup now stages and re-admits all seven fragments, dispatches prepare/rollback through fixed references to the separate owners before graphical setup, and retains the complete `Prepared` barrier without activation. Cross-layer and production-path tests cover identity/payload convergence and rollback at every failure position. Next place activation behind the launch gate and prove a separate visibility/recovery protocol before watched reload. |
