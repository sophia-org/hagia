...
| Launch, startup environment, configured processes, and shell supervision | Partial | Typed terminal/browser/startup/logout selectors resolve only against Sophia's registered applications. A bounded immutable overlay now centralizes CLI-superior application additions, arguments, startup order, and action mappings in one pure cloned-registry preparation seam. Port participant activation plus the remaining retained launch environment and supervision behavior. |
