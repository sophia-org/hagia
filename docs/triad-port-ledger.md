...
| Configuration discovery, validation, activation, reload, and rollback | Open | Discovery, partitioning, staging, policy preparation, and typed shortcut preparation exist. The generation/digest barrier retains a monotonic attempt counter and has executable TLA+ proofs for partial prepare, rollback, and stale completion; Sophia carries the matching coordinator, startup driver, and exact-key participant reducer. Cross-layer tests close unseen rollback and prove convergence or exact recovery divergence. Startup now loads raw provenance, exact key, and one identity-checked typed candidate bundle in one pass for future handlers. Populate those handlers behind the launch gate, then prove a separate global visibility/recovery protocol before watched reload. |
