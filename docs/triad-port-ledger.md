...
| Configuration discovery, validation, activation, reload, and rollback | Open | Discovery, partitioning, staging, policy preparation, and typed shortcut preparation exist. The generation/digest barrier retains a monotonic attempt counter and has executable TLA+ proofs for partial prepare, rollback, and stale completion; Sophia now carries the matching pure reducer and a startup-only driver that cancels failed batches and rolls all authorities back. Populate its handlers across dedicated authority protocols without a partial commit. |
