...
| Output mode, scale, position, transform, VRR, enablement, power, and reservations | Partial | Typed candidates reconcile purely against immutable capabilities; the existing atomic owner exposes read-only libdrm facts, a pure coordinator adapter joins them to Engine outputs without overclaiming scale/transform/VRR support, and native startup admits the result before client launch without activating it. Add atomic multi-output testing/apply, rollback, reservations, and a separate power authority. |
