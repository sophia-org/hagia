...
| Configuration discovery, validation, activation, reload, and rollback | Open | Discovery, partitioning, staging, policy preparation, and typed shortcut preparation exist. The generation/digest barrier retains a monotonic attempt counter and executable TLA+ proofs; Sophia carries the matching coordinator, startup driver, and exact-key participant reducer. Cross-layer tests run through authority-local payload slots and prove identity/payload convergence, last-known-good restoration, and recovery at every failure position. The same path now re-admits all seven exact owner-safe staged fragments and promotes their semantic payloads under one key. Populate production handlers behind the launch gate, then prove a separate global visibility/recovery protocol before watched reload. |
