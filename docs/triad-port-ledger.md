...
| Configuration discovery, validation, activation, reload, and rollback | Open | Discovery, partitioning, staging, policy preparation, and typed shortcut preparation exist. The generation/digest barrier retains a monotonic attempt counter and has executable TLA+ proofs for partial prepare, rollback, and stale completion; Sophia now carries the matching pure coordinator, a startup-only driver, and one reusable exact-key participant reducer. Cross-layer refinement tests close unseen rollback with a generation tombstone and prove convergence or exact recovery divergence across all authority failure positions. Populate handlers behind the startup launch gate, then prove a separate global visibility/recovery protocol before watched reload. |
