...
| Output mode, scale, position, transform, VRR, enablement, power, and reservations | Partial | Typed candidates now reconcile purely against immutable capabilities, including deterministic refresh matching and overlap/all-dark rejection, without touching DRM. Wire trusted topology snapshots, atomic multi-output activation, rollback, reservations, and a separate power authority. |
