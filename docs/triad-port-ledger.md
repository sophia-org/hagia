...
| Output mode, scale, position, transform, VRR, enablement, power, and reservations | Partial | Typed topology-independent candidates now validate exact connectors, modes, scale, position, transform, enablement, startup focus, and VRR without touching DRM. Add topology reconciliation, atomic multi-output activation, rollback, reservations, and a separate power authority. |
