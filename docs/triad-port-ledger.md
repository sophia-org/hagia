...
| Configuration discovery, validation, activation, reload, and rollback | Open | Discovery, partitioning, staging, policy preparation, and typed shortcut preparation exist. The generation/digest barrier retains a monotonic attempt counter and executable TLA+ proofs; Sophia carries the matching coordinator, startup driver, and exact-key participant reducer. Cross-layer tests now run through authority-local payload slots and prove identity/payload convergence, last-known-good restoration, and recovery at every failure position. Startup loads raw provenance, exact key, and one typed bundle in one pass; an owner-safe authority-fragment loader enforces section/key isolation. Populate production handlers behind the launch gate, then prove a separate global visibility/recovery protocol before watched reload. |
