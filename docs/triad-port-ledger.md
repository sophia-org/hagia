...
| Dialogs, transients, popups, and scratchpads | Complete | Reduced parent/role facts drive dialog defaults; standard and named scratchpads cycle and restore through bounded logical relations. Popup rendering remains outside WM policy. Dialog placement, visibility, stacking, and close fallback are ported from `tests/tcore_parented_popups.nim`, `tests/tcore_parented_geometry.nim`, and `tests/tcore_output_sticky_scratchpad.nim` at the reviewed baseline, with two deliberate divergences: position is derived per cycle from the parent's final geometry rather than stored at admission, and a dialog is never hidden merely because focus moved to an unrelated window. Triad's app-id, role, and startup rule matching stays with Sophia, as does spawn-context origin, which Triad resolves through PID ancestry. |
...
| Behavior family | State | Port requirement |
| --- | --- | --- |
| Overview and workspace previews | WM-owned implementation accepted headlessly, h002 | Layout, navigation and selection use Sophia's negotiated generic presentation/input contract. Paired t241 gates pass; physical acceptance remains separate. Hot corners, drag and animation behavior are outside this feature's scope. The earlier shell allocation is superseded by the [overview plan](notes/plans/64ac6jf6-workspace-overview-across-hagia-narthex-and-sophia.md). |
