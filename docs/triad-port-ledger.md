...
| Configuration discovery, validation, activation, reload, and rollback | Open | Discovery, partitioning, staging, policy preparation, and typed shortcut preparation exist. The generation/digest barrier retains a monotonic attempt counter and executable TLA+ proofs; Sophia carries the matching coordinator, startup driver, and exact-key participant reducer. Cross-layer tests prove convergence and recovery. Startup loads raw provenance, exact key, and one typed bundle in one pass; an owner-safe authority-fragment loader enforces section/key isolation. A generic authority-local slot now keeps participant identity and its bounded payload state synchronized for typed or admitted candidates without centralizing all authority state. Populate production handlers behind the launch gate, then prove a separate global visibility/recovery protocol before watched reload. |
