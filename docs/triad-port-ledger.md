...
| Configuration discovery, validation, activation, reload, and rollback | Open | Discovery, partitioning, staging, policy preparation, and typed shortcut preparation exist. The generation/digest barrier retains a monotonic attempt counter and executable TLA+ proofs; Sophia carries the matching coordinator, exact-key participant reducer, and separate prepare-only startup driver. Cross-layer tests prove identity/payload convergence, last-known-good restoration, and recovery at every failure position. Production startup creates a linear pre-graphical context that stages and re-admits all seven exact fragments; the prepare driver can stop at the complete barrier without activation. Connect production owner dispatch, then place activation behind the launch gate and prove a separate visibility/recovery protocol before watched reload. |
